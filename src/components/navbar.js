...
function Navbar({user}) {
  ...
  return (
    ...
    <div className="menu_bars">
      ...
      <div >
        <Link to='/'><AiFillHome size={30} className={activeNum==1?'icon-menu active':'icon-menu'} onClick={() => setActive(1)}/></Link>
        <Link to='/group'><RiGroup2Fill size={30} className={activeNum==2?'icon-menu active':'icon-menu'} onClick={() => setActive(2)}/></Link>
        <Link to='/doc'><IoMdDocument size={30} className={activeNum==3?'icon-menu active':'icon-menu'} onClick={() => setActive(3)}/></Link>
      </div>
